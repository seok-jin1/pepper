...
write.csv(perm_df, file.path(WORKDIR, "PERMANOVA_results.csv"), row.names = FALSE)
cat("  Saved: PERMANOVA_results.csv\n")
...
write.csv(sub_df, file.path(WORKDIR, "PERMANOVA_subsampling.csv"), row.names = FALSE)
cat("  Saved: PERMANOVA_subsampling.csv\n")
...
p2 <- ggplot(sub_plot_df, aes(x = F_val)) +
  ...
  geom_vline(xintercept = median(boot_F), color = "navy", lw = 1.2, linetype = "dashed") +
  geom_vline(xintercept = F_all, color = "red", lw = 1.2) +
